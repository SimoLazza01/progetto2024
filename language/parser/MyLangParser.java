...
import static language.parser.TokenType.*;
...
public class MyLangParser implements Parser {
	...
	private ForStmt parseForStmt() throws ParserException {
		...
		return new ForStmt(variable, exp, block);
	}
	...
	private Exp parseAtom() throws ParserException {
		return switch (tokenizer.tokenType()) {
		case NUM -> parseNum();
		case IDENT -> parseVariableInExp();
		case MINUS -> parseMinus();
		case OPEN_PAR -> parseRoundPar();
		case OPEN_DICT -> parseDict();
	...
	private Variable parseVariable() throws ParserException {
		...
		// 	var dict = parseDictChange(variable);
		// 	return dict;
		// }
		return variable;
	}

	private Exp parseVariableInExp() throws ParserException {
		final var name = tokenizer.tokenString();
		consume(IDENT); // this check is necessary for parsing correctly the 'var' statement
		var variable = new Variable(name);
		if (tokenizer.tokenType() == OPEN_DICT) {
	...
	private Exp parseDict() throws ParserException {
		...
		consume(CLOSE_DICT); 
		Exp dict = new DictCons(exp1, exp2);
		dict = parseDictChange(dict);
		return dict;
	}

	private Exp parseDictChange(Exp dict) throws ParserException {
		while (tokenizer.tokenType() == OPEN_DICT) {
			...
			if (tokenizer.tokenType() == DICT_OP) {
				...
				if (tokenizer.tokenType() == CLOSE_DICT) {
					dict = new DictDel(dict, exp1);
				} else {
					final var exp2 = parseExp();
					dict = new DictPut(dict, exp1, exp2);
				}
			} else {
				dict = new DictGet(dict, exp1);
			}
			consume(CLOSE_DICT); 
		}
		return dict;
